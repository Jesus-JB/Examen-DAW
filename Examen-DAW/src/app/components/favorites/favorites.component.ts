...
@Component({
  ...
  <div class="container mt-5">
    <h2 class="mb-4">Ciudades Favoritas</h2>

    <div *ngIf="!favoriteCities.length" class="text-center py-5">
      <i class="bi bi-star display-1 text-muted mb-3 d-block"></i>
      <h3 class="text-muted">Aún no tienes ciudades favoritas</h3>
      <p class="text-muted mb-4">Agrega ciudades a tus favoritas para consultar su clima rápidamente</p>
      <a routerLink="/search" class="btn btn-primary">
        <i class="bi bi-search me-2"></i>Buscar ciudades
      </a>
    </div>
                ...
                <div class="row g-2">
                  <div class="col-6">
                    <small>
                      ...
                      Máxima: {{ city.weather.main.temp_max | number:'1.0-0' }}°C
                    </small>
                  </div>
                  <div class="col-6">
                    <small>
                      <i class="bi bi-thermometer-low text-primary"></i>
                      Mínima: {{ city.weather.main.temp_min | number:'1.0-0' }}°C
                    </small>
                  </div>
                  <div class="col-6">
                    <small>
                      <i class="bi bi-moisture text-primary"></i>
                      Humedad: {{ city.weather.main.humidity }}%
                    </small>
                  </div>
                  <div class="col-6">
                    <small>
                      <i class="bi bi-wind text-primary"></i>
                      Viento: {{ city.weather.wind.speed }} m/s
                    </small>
                  </div>
                </div>
              </div>
  ...
  private loadWeatherForCities() {
    this.favoriteCities.forEach(city => {
      this.weatherService.getCurrentWeather(city.name).subscribe({
        ...
        error: (error) => {
          city.error = 'Error al cargar los datos del clima';
